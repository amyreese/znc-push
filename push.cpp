...
class CPushMod : public CModule
{
	...
	protected:
		...
		void send_message(const CString& message, const CString& title="New Message", const CString& context="*push", const CNick& nick=CString("*push"))
		{
			...
			else if (service == "toasty")
			{
				if (options["secret"] == "")
				{
					PutModule("Error: device id (secret) not set");
					return;
				}
                ...
                params["image"] = "https://github.com/exiva/znc-push/raw/supertoasty/logo.png";
